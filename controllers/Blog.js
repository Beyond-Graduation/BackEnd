...
router.get("/", isLoggedIn, async (req, res) => {
  ...
  if(req.query.blogId){
    ...
    blog = await Blog.findOne({blogId : curBlogId}).lean().catch((error) =>
...
router.post("/like", isLoggedIn, async (req, res) => {
  ...
  try {
    ...
    // else{
    //   blog.likes--
    //   blog = await Blog.updateOne({blogId: req.body.blogId  }, {likes:blog.likes});
    //   user.likedBlogs.push(req.body.blogId)
    // }
    res.json(blog);
...
router.post("/addComments", isLoggedIn, async (req, res) => {
  ...
  try {
    ...
    if(req.body.parent){
      var parentComment= await BlogComments.findOne({commentId: req.body.parent })
...
router.get("/getComments", isLoggedIn, async (req, res) => {
  ...
  if(req.query.parentId){
    ...
    res.json(
      await BlogComments.find({parent : curCommentId}).lean().catch((error) =>
        res.status(400).json({ error })
      )
    );
  }
    ...
    res.json(
      await BlogComments.find({parent : null}).lean().catch((error) =>
        res.status(400).json({ error })
      )
    );
}
