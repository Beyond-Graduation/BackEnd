...
router.post("/reset_password", async(req, res) => {
    const { User } = req.context.models;
    let user = await User.findOne({ passwordResetToken: req.body.passwordResetToken }).lean();
    if (user) {
        req.body.updated = Date.now();
        req.body.newPassword = await bcrypt.hash(req.body.newPassword, 10);
        ...
        user = await User.findOne({ passwordResetToken: req.body.passwordResetToken }).lean();
        ...
        res.status(400).send("Invalid Password Reset link!, Kindly proceed to forgot password again for a new link")
    }
});
